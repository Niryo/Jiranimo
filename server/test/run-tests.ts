...
async function checkPrerequisites(): Promise<boolean> {
  ...
  if (existsSync(resolve(SERVER, 'node_modules'))) {
    ...
    if (answer.toLowerCase() !== 'n') {
      ...
      fail('Dependencies required — run: cd server && npm install');
      ok = false;
    }
  }
...
async function runUnitTests(): Promise<boolean> {
  ...
  if (!ok) {
    ...
    1. Run ${CYAN}cd server && npx vitest run${RESET} to see full output
...
async function runIntegrationTests(): Promise<boolean> {
  ...
  if (!ok) {
    log(`
    ${BOLD}How to fix:${RESET}
    1. Run ${CYAN}cd server && npx vitest run --config vitest.integration.config.ts${RESET}
...
async function runE2eTests(): Promise<boolean> {
  ...
  if (!ok) {
    log(`
    ${BOLD}How to fix:${RESET}
    1. Run ${CYAN}cd server && npx vitest run --config vitest.e2e.config.ts${RESET}
